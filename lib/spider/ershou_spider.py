...
class ErShouSpider(BaseSpider):
    ...
    def get_area_ershou_info(city_name, area_name):
        ...
        district_name = area_dict.get(area_name, "")
        # 中文区县
        chinese_district = get_chinese_district(district_name) or district_name
        # 中文版块
        chinese_area = chinese_area_dict.get(area_name, "") or area_name
        ...
        for num in range(1, total_page + 1):
            ...
            for house_elem in house_elements:
                ...
                name = re.sub('\s','',name)
                pic = pic.get('data-original').strip().replace(',', "，")
                # 继续分拣desc
                ...
                for desc_item in desc:
                    # print(desc_item)
                    if desc_item.find("楼层") > -1:
                        floor = desc_item
                    if desc_item.find("室") > -1:
                        house_type = desc_item
                    if desc_item.find("平米") > -1:
                        size = desc_item

                # 作为对象保存
                ershou = ErShou(chinese_district, chinese_area, name, community, price, floor, house_type, direction, size, pic)
